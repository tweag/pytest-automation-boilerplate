...
from bp_core.lib.pytest_testrail_client._enums import METHODS
from bp_core.lib.pytest_testrail_client._exception import TestRailError
from bp_core.lib.pytest_testrail_client.model.case import Case
from bp_core.lib.pytest_testrail_client.model.case_type import CaseType
from bp_core.lib.pytest_testrail_client.model.plan import Entry, Plan
from bp_core.lib.pytest_testrail_client.model.priority import Priority
from bp_core.lib.pytest_testrail_client.model.project import Project
from bp_core.lib.pytest_testrail_client.model.result import Result
from bp_core.lib.pytest_testrail_client.model.section import Section
from bp_core.lib.pytest_testrail_client.model.status import Status
from bp_core.lib.pytest_testrail_client.model.suite import Suite
from bp_core.lib.pytest_testrail_client.model.templates import Template
from bp_core.lib.pytest_testrail_client.model.test import Test
...
class Cases(BaseCategory):
    ...
    def get_cases(self, project_id: int, **kwargs) -> List[Case]:
        ...
        return [Case(rsp) for rsp in response["cases"]]
    ...
    def update_case(self, case_id: int, case: Case) -> Case:
        ...
        data = case.raw_data()
        response = self._session.request(
            METHODS.POST, f"update_case/{case_id}", json=data
        )
        if "error" in response:
            raise TestRailError("Case update failed with error: %s" % response["error"])
        return Case(response)

    def update_case_field(self, case_id: int, **kwargs):
        """
        Updates an existing test case field (partial updates are supported, i.e.
        you can submit and update specific fields only).
        :param case_id: The ID of the test case (required)
        """
        data = dict(**kwargs)
        ...
        if "error" in response:
            raise TestRailError("Case field update failed with error: %s" % response["error"])
...
class Plans(BaseCategory):
    ...
    def get_plans(self, project_id: int, **kwargs) -> List[Plan]:
        ...
        return [Plan(obj) for obj in response["plans"]]
    ...
    def update_plan(self, plan: Plan) -> Plan:
        ...
        data = plan.raw_data()
        response = self._session.request(
            METHODS.POST, f"update_plan/{plan.id}", json=data
        )
        return Plan(response)
    ...
    def update_plan_entry(self, plan_id: int, entry: Entry) -> Entry:
        ...
        data = entry.raw_data()
        response = self._session.request(
            METHODS.POST, f"update_plan_entry/{plan_id}/{entry.id}", json=data
        )
        return Entry(response)
...
class Projects(BaseCategory):
    ...
    def get_projects(self, **kwargs) -> List[Project]:
        ...
        return [Project(obj) for obj in response["projects"]]
    ...
    def add_project(self, project: Project) -> Project:
        ...
        if "error" in response:
            raise TestRailError(
                "Project creation failed with error: %s" % response["error"]
            )
        return Project(response)
    ...
    def update_project(self, project: Project) -> Project:
        ...
        data = project.raw_data()
        response = self._session.request(
            METHODS.POST, f"update_project/{project.id}", json=data
        )
        if "error" in response:
            raise TestRailError(
                "Project update failed with error: %s" % response["error"]
            )
        return Project(response)
    ...
    def delete_project(self, project_id: int):
        ...
        if "error" in response:
            raise TestRailError(
                "Project delete failed with error: %s" % response["error"]
            )
        return response
...
class Results(BaseCategory):
    ...
    def add_result(self, result: Result) -> Result:
        ...
        response = self._session.request(
            METHODS.POST, f"add_result/{result.test_id}", json=data
        )
        return Result(response)

    def add_result_for_case(
        self, run_id: int, case_id: int, result: Result
    ) -> List[Result]:
        """
        ...
        result = self._session.request(
            METHODS.POST, f"add_result_for_case/{run_id}/{case_id}", json=data
        )
    ...
    def add_results(self, run_id: int, results: List[Result]) -> List[Result]:
        ...
        response = self._session.request(
            METHODS.POST, f"add_results/{run_id}", json=payload
        )
    ...
    def add_results_for_cases(self, run_id: int, results: List[Result]) -> List[Result]:
        ...
        response = self._session.request(
            METHODS.POST, f"add_results_for_cases/{run_id}", json=data
        )
        return [Result(obj) for obj in response]
...
class Sections(BaseCategory):
    ...
    def get_sections(self, project_id: int, suite_id: int) -> List[Section]:
        ...
        """
        response = self._session.request(
            METHODS.GET, f"get_sections/{project_id}", params={"suite_id": suite_id}
        )
    ...
    def add_section(self, project_id: int, section: Section) -> Section:
        ...
        if "error" in response:
            raise TestRailError(
                "Section creation failed with error: %s" % response["error"]
            )
        return Section(response)
    ...
    def update_section(self, section: Section) -> Section:
        ...
        data = section.raw_data()
        response = self._session.request(
            METHODS.POST, f"update_section/{section.id}", json=data
        )
        if "error" in response:
            raise TestRailError(
                "Section update failed with error: %s" % response["error"]
            )
        return Section(response)
...
class Suites(BaseCategory):
    ...
    def add_suite(self, project_id: int, suite: Suite) -> Suite:
        ...
        if "error" in response:
            raise TestRailError(
                "Suite creation failed with error: %s" % response["error"]
            )
        return Suite(response)
    ...
    def update_suite(self, suite: Suite) -> Suite:
        ...
        data = suite.raw_data()
        response = self._session.request(
            METHODS.POST, f"update_suite/{suite.id}", json=data
        )
        if "error" in response:
            raise TestRailError(
                "Suite update failed with error: %s" % response["error"]
            )
        return Suite(response)
    ...
    def delete_suite(self, suite_id: int):
        ...
        if "error" in response:
            raise TestRailError(
                "Suite delete failed with error: %s" % response["error"]
            )
        return response
